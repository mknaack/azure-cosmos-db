...
let create_database_if_not_exists_with_partition_key_test _ () =
  let%lwt res = D.create_if_not_exists dbname_partition in
  match res with
  | Result.Ok (code, body) ->
...
let list_databases_with_partition_key_test _ () =
...
let list_databases_with_partition_key_timeout_test _ () =
...
let get_database_with_partition_key_test _ () =
  let%lwt res = D.get dbname_partition in
  match res with
  | Result.Ok (code, body) ->
      let _ = Alcotest.(check int) "Status same int" 200 code in
      let _ =
...
let get_database_with_partition_key_timeout_test _ () =
  let%lwt res = D.get ~timeout:0.0 dbname_partition in
  match res with
  | Result.Error Timeout_error ->
      Alcotest.(check unit) "Timeout error" () ();
      Lwt.return_unit
...
let create_database_if_not_exists_with_partition_key_test_new_database _ () =
...
let create_collection_with_partition_key_if_not_exists_test_new_collection _ ()
    =
  ...
  let%lwt () =
    ...
    | Result.Error _ -> Alcotest.fail "Should not return error"
  in
  match%lwt D.Collection.delete dbname_partition collection_name_test with
  | Result.Error _ -> Alcotest.fail "Should not return error"
  | Result.Ok code ->
      let _ = Alcotest.(check int) "Status same int" 204 code in
      Lwt.return_unit

let list_collection_with_partition_key_test _ () =
  let%lwt res = D.Collection.list dbname_partition in
  match res with
...
let get_collection_with_partition_key_test _ () =
  let%lwt res = D.Collection.get dbname_partition collection_name_partition in
  match res with
  | Result.Error _ -> Alcotest.fail "Should not return error"
  | Result.Ok (code, body) ->
      let _ = Alcotest.(check int) "Status same int" 200 code in
...
let list_document_with_partition_key_test _ () =
  let%lwt res =
  ...
  | Result.Ok (code, _, { rid = _; documents; count }) ->
      let _ = Alcotest.(check int) "Status same int" 200 code in
      let _ = Alcotest.(check bool) "Count" true (count > 0) in
      let _ = Alcotest.(check int) "Count list" count (List.length documents) in
      Lwt.return_unit

let list_multiple_documents_with_partition_key_test _ () =
  let list_values =
...
let change_feed_with_partition_key_test _ () =
  let%lwt res =
...
let get_document_with_partition_key_test _ () =
  let%lwt res =
...
let create_a_lot_of_documents_with_partition_key_test _ () =
...
let test_partition_key_cosmos =
  [
    ...
    Alcotest_lwt.test_case "list database" `Slow
      list_databases_with_partition_key_test;
    Alcotest_lwt.test_case "list database timeout" `Slow
      list_databases_with_partition_key_timeout_test;
    Alcotest_lwt.test_case "get database" `Slow
      get_database_with_partition_key_test;
    Alcotest_lwt.test_case "get database timeout" `Slow
      get_database_with_partition_key_timeout_test;
    ...
    Alcotest_lwt.test_case "create collection with partition key if not exists"
      `Slow create_collection_with_partition_key_if_not_exists_test;
    ...
    Alcotest_lwt.test_case "list collection with partition key" `Slow
      list_collection_with_partition_key_test;
    Alcotest_lwt.test_case "get collection with partition key" `Slow
    ...
    Alcotest_lwt.test_case "list document with partition key" `Slow
      list_document_with_partition_key_test;
    Alcotest_lwt.test_case "list multiple documents with partition key" `Slow
      list_multiple_documents_with_partition_key_test;
    Alcotest_lwt.test_case "change feed with partition key" `Slow
      change_feed_with_partition_key_test;
    ...
    Alcotest_lwt.test_case "get document with partition key" `Slow
      get_document_with_partition_key_test;
    ...
    Alcotest_lwt.test_case "create a lot document with partition key" `Slow
