...
module type Account = sig
  ...
  val authorization : verb -> string -> string -> string
  val endpoint : string
end
...
module Auth (Keys : Auth_key) : Account = struct
  ...
  let authorization verb date db_name = (* "type=master&ver=1.0&sig=" ^ key *)
...
module Database (Account : Account) = struct
  let headers verb db_name =
    let ms_date =
      let now = Unix.time () in
      Utility.x_ms_date now
      in
      Http_headers.empty
      |> Http_headers.add (Http_headers.name "authorization") (Account.authorization verb ms_date db_name)
