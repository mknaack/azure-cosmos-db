...
module Database (Auth_key : Auth_key) = struct
  ...
  let result_or_error_with_result expected_code f code =
    if code = expected_code then Result.ok (expected_code, f ()) else Result.error (Azure_error code)

  let result_or_error expected_code code =
    if code = expected_code then Result.ok expected_code else Result.error (Azure_error code)
  ...
  let delete ?timeout name =
    let uri = Uri.make ~scheme:"https" ~host ~port:443 ~path:("dbs/" ^ name) () in
    let response = Cohttp_lwt_unix.Client.delete ~headers:(headers Account.Dbs Account.Delete ("dbs/" ^ name)) uri >>= Lwt.return_some |> wrap_timeout timeout in
    match%lwt response with
    | None -> timeout_error
    | Some (resp, _) ->
      let code = get_code resp in
  ...
  module Collection = struct
    ...
    let create_if_not_exists ?(indexing_policy=None) ?(partition_key=None) ?timeout dbname coll_name =
      ...
      match exists with
      | Ok (404, _) -> create ?timeout ~indexing_policy ~partition_key dbname coll_name (* TODO: should this be error 404 *)
    ...
    module Document = struct
      ...
      let list ?max_item_count ?continuation ?consistency_level ?session_token ?a_im ?if_none_match ?partition_key_range_id ?timeout dbname coll_name =
        ...
        in
        let get_action = Cohttp_lwt_unix.Client.get ~headers uri >>= Lwt.return_some |> wrap_timeout timeout in
  ...
  module User = struct
    ...
    let list ?timeout dbname =
      ...
      let response = Cohttp_lwt_unix.Client.get ~headers:(headers Account.Get header_path) uri >>= Lwt.return_some |> wrap_timeout timeout in
      match%lwt response with
      | None -> timeout_error
      | Some (resp, body) ->
        let code = get_code resp in
    ...
    let replace ?timeout dbname user_name new_user_name =
      ...
      let header_path = "dbs/" ^ dbname ^ "/users/" ^ user_name in
      let uri = Uri.make ~scheme:"https" ~host ~port:443 ~path () in
      let response = Cohttp_lwt_unix.Client.put ~headers:(headers Account.Put header_path) ~body uri >>= Lwt.return_some |> wrap_timeout timeout in
      match%lwt response with
      | None -> timeout_error
      | Some (resp, body) ->
        let code = get_code resp in
    ...
    let delete ?timeout dbname user_name =
      ...
      let header_path = "dbs/" ^ dbname ^ "/users/" ^ user_name in
      let uri = Uri.make ~scheme:"https" ~host ~port:443 ~path () in
      let response = Cohttp_lwt_unix.Client.delete ~headers:(headers Account.Delete header_path) uri >>= Lwt.return_some |> wrap_timeout timeout in
      match%lwt response with
      | None -> timeout_error
      | Some (resp, _body) ->
        let code = get_code resp in
