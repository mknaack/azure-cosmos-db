...
module Database (Auth_key : Auth_key) = struct
  ...
  module User = struct
    ...
    let list dbname =
      ...
      let uri = Uri.make ~scheme:"https" ~host ~port:443 ~path () in
      Cohttp_lwt_unix.Client.get ~headers:(headers resource Account.Get ("dbs/" ^ dbname)) uri >>= fun (resp, body) ->
      let code = get_code resp in
      body |> Cohttp_lwt.Body.to_string >|= fun body ->
    ...
    let get dbname user_name =
      ...
      let uri = Uri.make ~scheme:"https" ~host ~port:443 ~path () in
      Cohttp_lwt_unix.Client.get ~headers:(headers resource Account.Get header_path) uri >>= fun (resp, body) ->
      let code = get_code resp in
      body |> Cohttp_lwt.Body.to_string >|= fun body ->
      let value = match code with
    ...
    let delete dbname user_name =
      ...
      let header_path = "dbs/" ^ dbname ^ "/users/" ^ user_name in
      let uri = Uri.make ~scheme:"https" ~host ~port:443 ~path () in
      Cohttp_lwt_unix.Client.delete ~headers:(headers resource Account.Delete header_path) uri >>= fun (resp, body) ->
      let code = get_code resp in
      body |> Cohttp_lwt.Body.to_string >|= fun _ ->
      code
  end
